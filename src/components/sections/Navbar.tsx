			...
			<header className={"px-8 lg:px-10 py-4 lg:py-6 fixed top-0 w-full z-50"}>
				<nav className={"flex justify-between items-center box-border"}>
					<div className="relative" style={{width: 'clamp(326px, 45vw, 550px)'}}>
						...
						<div className="absolute z-60 top-6 md:top-8 lg:top-10 w-full">
					...
					<div className={"hidden md:flex gap-4 text-[7px] md:text-[8px] lg:text-[10px] font-semibold"}>
					...
					<button
						...
						className={`flex items-center gap-2 ${isOpen ? "bg-warm-cream text-walnut-shadow " : "border-dotted"} md:hidden px-[1.75em] py-[1em] border-[1.4px] rounded-full cursor-pointer transition-all duration-200`}>
						<div
							className={`w-2 h-2 lg:w-2.5 lg:h-2.5 rounded-full ${isOpen ? "bg-walnut-shadow" : "bg-warm-cream"}`}/>
						<p className={"uppercase text-[20px] font-semibold tracking-tighter scale-y-105"}>{isOpen ? "Close" : "Menu"}
						</p>
					</button>
				</nav>
			</header>
